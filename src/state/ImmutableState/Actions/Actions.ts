...
export class Actions {
  ...
  public move(player: IFieldPlayer, type: string, ref: IBlock): ISituation {
    ...
    if (playerFunc.findFreeBlock(around) !== undefined) {
      switch (type) {
        ...
        case 'forward':
          const opponentBlock = this.findMarkingOpponent(player, around);
  ...
  public shoot(player: IFieldPlayer, post: IBlock, reason: string) {
    ...
    const result = this.decider.getShotResult(player, keeper);
  ...
  private makeMove(
    ...
    switch (path.x) {
      case -1:
        const b1 = around.left;
        if (b1.occupant == null) {
          player.move(path);
          return true;
        } else {
          const b = playerFunc.findFreeBlock(around);
          if (b === undefined) {
            return false;
          } else {
